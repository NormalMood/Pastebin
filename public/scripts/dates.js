...
if (datetimeMetadataExpiresAt !== null) {
    if (datetimeMetadataExpiresAt.textContent === 'infinity') {
        ...
        datetimeMetadataExpiresAt.setAttribute('title', 'Пост будет удален: Не удалять');
        datetimeMetadataExpiresAtImg.setAttribute('title', 'Пост будет удален: Не удалять');
    } else {
        ...
        if (minutesLeft === 0) {
            ...
            datetimeMetadataExpiresAt.textContent = minutesLeft + ' мин.';
        } else if (daysLeft === 0) {
            datetimeMetadataExpiresAt.textContent = hoursLeft + ' ч.';
        } else if (weeksLeft === 0) {
            datetimeMetadataExpiresAt.textContent = daysLeft + ' д.';
        } else if (monthsLeft === 0) {
            datetimeMetadataExpiresAt.textContent = weeksLeft + ' нед.';
